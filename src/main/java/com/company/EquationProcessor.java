...
public class EquationProcessor {
    ...
    private int processForUnaryMinus(char[] equationParts, int index) {
        if (equationParts[index] == '-') {
            boolean ifExistUnaryMinus = (index >= 1
                    && (isOperator(equationParts[index - 1]) || equationParts[index - 1] == '('))
                    || index == 0;
    ...
    private void calculateAndPopLessPriority(Operator operator) {
        while (!operationsStack.isEmpty() && !operationsStack.peek().isOpenParentheses() &&
    ...
    public void checkInput(String equation) {
        ...
        String operations = "[*/^]";
        String plusMinus = "[-+]";
        pattern = Pattern.compile(operations + "[ ]*" + operations
                + "|" + plusMinus + "[ ]*" + plusMinus + "[ ]*" + plusMinus
                + "|" + plusMinus + "[ ]*" + operations);
